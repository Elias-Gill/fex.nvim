...
local function preview(ctx, cmd)
	...
	view(
		...
		function(path)
			maintainWindows(ctx, cmd .. " " .. path, function() end)
		end,
		-- Open directory in preview window
		function(path)
			maintainWindows(ctx, cmd, function()
				M.open(path)
			end)
		end
	)
...
M.changeCwd = function()
	local curr = getCurrent(ctxFromCurrent())
	vim.api.nvim_set_current_dir(curr.root.name)
	print("Changing CWD")
end
...
M.setup = function(options)
	...
	-- Enables opening a directory directly inside Fex on startup
